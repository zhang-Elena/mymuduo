...
/**
 * 发送数据  防止应用写的快，而内核发送数据慢，需要把待发送数据写入缓冲区中，并设置水位回调
*/
void TcpConnection::sendInLoop(const void* data, size_t len){
    ...
    if(!channel_->isWriting() && outputBuffer_.readableBytes() == 0){
        ...
        if(nwrote >= 0){
            ...
            //一次性数据发送完，就不用再给channel_设置epollout事件
    ...
    //然后给channel注册epollout事件，poller发送tcp的发送缓冲区有空间，
    //则会通知对应的sock-channel，调用writeCallback回调方法
...
void TcpConnection::connectDestroyed(){
    if(state_ == kConnected){
        ...
        channel_->disableAll(); //把channel所有感兴趣的事件，从poller中del
        connectionCallback_(shared_from_this());
    }
